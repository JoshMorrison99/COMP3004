...
class Lesson005: UIViewController, Subscriber {
    ...
    func startLesson(){
        ...
        LessonLabel.text = "Hello! Welcome to Lesson 5 of the Pitch Pal App. In this lesson, we are going to learn the basics of the Pitch Pal App! To proceed tap anywhere on the screen!"
        LessonLabel_number.text = "1 / 3"
    ...
    @objc func step001(_ sender: UITapGestureRecognizer? = nil) {
        ...
        clearNotes()
        LessonLabel.text = "Pitch Pal works by picking up the sound through your mobile device's microphone."
        LessonLabel_number.text = "2 / 3"
        lessonModel.accumulateLessonStepNumber()
    ...
    @objc func step002(_ sender: UITapGestureRecognizer? = nil) {
        // handling code
        LessonLabel.text = "Play the following sequence of notes on your piano. Remember what you have learned so far!"
    ...
    func lessonLoop(){
        Timer.scheduledTimer(withTimeInterval: lessonModel.getTimer(), repeats: true) { timer in
            if(self.lessonModel.getLessonStepNum() == 3){
                self.LessonLogic()
            }
        }
    }
